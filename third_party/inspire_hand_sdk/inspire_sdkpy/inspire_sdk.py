...
class ModbusDataHandler:
    def __init__(self, data=data_sheet, history_length=100, network=None, ip=None, port=6000, device_id=1, LR='r', use_serial=False, serial_port='/dev/ttyUSB0', baudrate=115200, states_structure=None, initDDS=True, max_retries=5, retry_delay=2):
       ...
       # 初始化 ChannelFactory
        ...
        self.client.write_register(1004, 1, slave=self.device_id) #reser error
    ...
    def write_registers_callback(self,msg:inspire_hand_ctrl):
        with modbus_lock:
            if msg.mode & 0b0001:  # 模式 1 - 角度
                self.client.write_registers(1486, msg.angle_set, slave=self.device_id)
                # print('angle_set')
            if msg.mode & 0b0010:  # 模式 2 - 位置
                self.client.write_registers(1474, msg.pos_set, slave=self.device_id)
                # print('pos_set')

            if msg.mode & 0b0100:  # 模式 4 - 力控
                self.client.write_registers(1498, msg.force_set, slave=self.device_id)
                # print('force_set')

            if msg.mode & 0b1000:  # 模式 8 - 速度
                self.client.write_registers(1522, msg.speed_set, slave=self.device_id)
    ...
    def read_and_parse_registers(self, start_address, num_registers, data_type='short'):
         with modbus_lock:
            # 读取寄存器
            response = self.client.read_holding_registers(start_address, num_registers, slave=self.device_id)
